...
export class EditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if(this.user){
      ...
      const foundAdmin = this.user.roles.find(element => element == 'ADMIN');
      if(foundAdmin){
        this.isAdmin = true;
      }
  ...
  async submit(){
    ...
    if(data.suspended !== this.user.hasOwnProperty('suspendedAt')){
      await this.usersService.suspend(this.user.id, data.suspend).toPromise()
    }
    this.usersService.update(this.user.id,data).subscribe((resp:any)=>{
        this.notificationsService.showSuccess('User edited!');
