...
export class DevicesComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  checkout(device){
    swal.fire({
      title: 'Checkout',
      text: 'Are you sure you want to check out this device?',
      type: 'warning',
      buttonsStyling: true,
      ...
      if (data && data.value) {
        this.devicesService.checkout(device.id).subscribe(()=>{
          this.notificationsService.showSuccess("success.checked_out");
          this.pageChanged();
        }, (res) => {
  ...
  checkin(device){
    swal.fire({
      title: 'Checkin',
      text: 'Are you sure you want to check in this device?',
      type: 'warning',
      buttonsStyling: true,
      ...
      if (data && data.value) {
        this.devicesService.checkin(device.id).subscribe(()=>{
          this.notificationsService.showSuccess("success.checked_in");
          this.pageChanged();
        }, (res) => {
          if (res.error && res.error.message) {
            this.notificationsService.showError(res.error.message);
