...
@Component({
  ...
  templateUrl: 'email-set-password.component.html'
})
export class EmailSetPasswordComponent implements OnInit {
  submitted = false;
  token = null;
  ...
  submit(){
    ...
    this.authService.resetPassword(data).subscribe((response:any) => {
      this.notificationsService.showSuccess("success.set_password_complete");
